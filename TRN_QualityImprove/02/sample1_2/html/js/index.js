var NOWSCREEN = '#screen_top';/* 現在表示中の画面を保持する変数 */
var PRESCREEN = '';/* 一つ前の画面を保持する変数 */
...
QiSession(function(session) {
    session.service('ALMemory').then(function(ALMemory){
        ...
        $('.salue').on('touchend', function(){
            /* 発話中のセリフをSTOPする */
            ALMemory.raiseEvent("sample_app/stop_speech", 1).then(function(){
                /* 挨拶用セリフを発話指示 */
        ...
        function switchDisplay(currentDiv, nextDiv){
            ...
            ALMemory.raiseEvent("sample_app/stop_speech", 1).then(function(){
                /* 画面表示セリフを発話する */
                setTimeout(function(){ALMemory.raiseEvent("sample_app/speech", returnSpeechText)}, 1500);
            });            
        }
    });
});
